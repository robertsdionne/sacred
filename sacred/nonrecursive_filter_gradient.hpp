...
public:
  ...
  void operator ()(const tensor_type &delta, const tensor_type &x, tensor_type &x_gradient) {
    ...
    // ∇_c E = ∇_y E ⁎ u
    math_.BackwardNarrowConvolve2(filter_gradient_, x, delta, F(1.0), F(1.0));

    // Bias derivatives:
    // ∂E/∂b_n = ∂E/∂y_n
    // ∇_b E = ∇_y E
    math_.Add(bias_gradient_, delta, F(1.0), F(1.0));

    // Bottom derivatives:
    // ∂E/∂u_n = Σ_k ∂E/∂y_{n+k} c_k
    // ∇_u E = ∇_y E ⁎ c
    math_.BackwardWideConvolve2(x_gradient, filter_, delta, F(1.0), F(1.0));
  }
